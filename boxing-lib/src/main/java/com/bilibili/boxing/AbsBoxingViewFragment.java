...
public abstract class AbsBoxingViewFragment extends Fragment implements PickerContract.View {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (mCameraPicker != null && (requestCode == CameraPickerHelper.REQ_CODE_CAMERA || requestCode == CameraPickerHelper.REQ_CODE_VIDEO)) {
    ...
    public final void startCamera(Activity activity, Fragment fragment, String subFolderPath) {
        try {
            if (!BoxingBuilderConfig.TESTING && ContextCompat.checkSelfPermission(getActivity(), CAMERA_PERMISSIONS[0]) != PERMISSION_GRANTED) {
                ...
                if (!BoxingManager.getInstance().getBoxingConfig().isVideoMode()) {
                    mCameraPicker.startCamera(activity, fragment, subFolderPath, CameraPickerHelper.REQ_CODE_CAMERA);
                } else {
                    mCameraPicker.startCamera(activity, fragment, subFolderPath, CameraPickerHelper.REQ_CODE_VIDEO);
                }
            }
        } catch (IllegalArgumentException | IllegalStateException e) {
    ...
    private static final class CameraListener implements CameraPickerHelper.Callback {
        ...
        @Override
        public void onFinish(@NonNull CameraPickerHelper helper, int requestCode) {
            ...
            if (!file.exists()) {
                onError(helper, requestCode);
                return;
            }
            BaseMedia media = null;
            if (requestCode == CameraPickerHelper.REQ_CODE_CAMERA) {
                media = new ImageMedia(file);
                ((ImageMedia) media).saveMediaStore(fragment.getAppCr());
            } else if (requestCode == CameraPickerHelper.REQ_CODE_VIDEO) {
                media = new VideoMedia(file);
                ((VideoMedia) media).saveMediaStore(fragment.getAppCr());
            }
            fragment.onCameraFinish(media);
        }

        @Override
        public void onError(@NonNull CameraPickerHelper helper, int requestCode) {
            AbsBoxingViewFragment fragment = mWr.get();
            if (fragment == null) {
                return;
            }

...
//		MyApplication.setLocaleVi(context);
	...
	public void setReferenceTime(long referenceTime) {
		...
		this.mStrReferenceTime = convertLongToString(this.mReferenceTime);
		/*
	...
	private Long convertDateToLong(String strReferenceTime) {
		...
		SimpleDateFormat f = new SimpleDateFormat(SERVER_FORMAT);
		f.setTimeZone(TimeZone.getTimeZone(TIME_ZONE));
		Date d = new Date();
		try {
	...
	private String convertLongToString(Long longTime) {
		if(longTime == null) {
			return null;
		}
		SimpleDateFormat f = new SimpleDateFormat(SERVER_FORMAT);
		...
		try {
			strReferenceTime = f.format(d);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return strReferenceTime;
	}

	private String convertTo12hFormat(String strReferenceTime) {
		String formattedDate = "";
		SimpleDateFormat readFormat = new SimpleDateFormat(
				SERVER_FORMAT);
		readFormat.setTimeZone(TimeZone.getTimeZone(TIME_ZONE));
		SimpleDateFormat writeFormat = new SimpleDateFormat("h:mm aa");
		Date date = null;
		try {
...
//		Locale viLocale = new Locale(Constants.VI_LANGUAGE, Constants.VI_COUNTRY);
		...
		SimpleDateFormat readFormat = new SimpleDateFormat(
				SERVER_FORMAT);
		readFormat.setTimeZone(TimeZone.getTimeZone(TIME_ZONE));
		SimpleDateFormat writeFormat = new SimpleDateFormat("dd MMM yyyy 'at' h:mm aa");
		Date date = null;
		try {
			date = readFormat.parse(strReferenceTime);
		} catch (ParseException e) {
...
//		Locale viLocale = new Locale(Constants.VI_LANGUAGE, Constants.VI_COUNTRY);
	...
	private CharSequence getRelativeTimeDisplayString() {
		...
		if (difference <= DateUtils.MINUTE_IN_MILLIS) {
			...
			result = DateUtils.getRelativeTimeSpanString(mReferenceTime, now,
					DateUtils.DAY_IN_MILLIS, DateUtils.FORMAT_ABBREV_RELATIVE) + convertTo12hFormat(mStrReferenceTime);
